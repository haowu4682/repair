...
def nrsucc(r):
  """Check whether the syscall described by record 'r' was
  successful."""
...
# @util.memorize
def nrdep(r):
  """Get (rs, ws) for a record 'r', where 'rs' are the items that the
  syscall described by 'r' read, and 'ws' are the items that it wrote.
  """
  rs = set()
  ws = set()
  sc = syscalls[r.nr]
