	...
	def __repr__(self):
		...
		if self.fd >= 0:
			...
			if hasattr(self, "offset"):
				r += "@" + ("a" if self.offset == -1 \
					    else repr(self.offset))
		return r
...
def sysarg_inode(f, arg = None):
	...
	if mode == 0:
		return None
	i = _inode(mode)
	setattr(i, "dev", sysarg_dev_t(f))
	setattr(i, "ino", sysarg_ino_t(f))
	if S_ISCHR(mode) or S_ISBLK(mode):
		setattr(i, "rdev", sysarg_dev_t(f))
	else:
		setattr(i, "rdev", 0)
	return i
...
class PathidTable:
	...
	def dumptables():
		...
		for k,v in PathidTable.pdt.pathid_map.iteritems():
			for kk in k: sys.stdout.write("%02x" % ord(kk))
			print ' ',v[0], v[1],' ',
			for kk in v[2]: sys.stdout.write("%02x\n" % ord(kk))
		print
		print 'inode_map:'
		for k,v in PathidTable.pdt.inode_map.iteritems(): print k,v
	def __init__(self, dirp):
		self.pathid_map = {}
		self.inode_map = defaultdict(set)
		self.dirp = dirp
		files = os.listdir(dirp)
		syscallN = [os.path.join(dirp, x) for x in files if re.match("^syscall[0-9]$", x)]
		pathidtableN = [os.path.join(dirp, x) for x in files if re.match("^pathidtable[0-9]$", x)]
		...
		for fn in pathidtableN:
			...
			for k,v in db.items():
				value = self._parsevalue(v)
				parents.add(v[2])
				self.pathid_map[k] = value
		for k in set(self.pathid_map.keys()).difference(parents):
			pathids = [k]
			v = self.pathid_map[k]
			self.inode_map[v[1].dev ^ (v[1].ino & 0xffffffff)].update(pathids)
			while v[2] != 20*'\0':
				pathids.append(v[2])
				v = self.pathid_map[v[2]]
				self.inode_map[v[1].dev ^ (v[1].ino & 0xffffffff)].update(pathids)
	def _parsevalue(self, value):
		if value is not None:
			pathid_parent = value[:20]
			lf = util.ListFile(value[20:])
			inode = sysarg_inode(lf)
			name = value[20+lf.i:]
			return (name, inode, pathid_parent)
	def _expand(self, key):
		p = []
		while key in self.pathid_map:
			value = self.pathid_map[key]
			p.append(dentry(value[1], value[0]))
			key = value[2]
		return p
	def _getpathids(self, dev_xor_ino):
		return self.inode_map[dev_xor_ino] if dev_xor_ino in self.inode_map else None
