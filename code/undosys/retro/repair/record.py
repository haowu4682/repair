...
def _read_syscall(f):
	...
	setattr(r, "pid", sysarg_uint(f))
	setattr(r, "usage", sysarg_uint(f))
	assert r.usage in [0, 1]
	setattr(r, "nr", sysarg_uint(f))
	sc = syscalls[r.nr]
	setattr(r, "sid", _read_sid(f))
	setattr(r, "args", {})
	args = []
...
class IndexLoader:
	...
	def _get_records(self, keytype, key):
		...
		for (fidx, offset, nr, sid) in self._dbget(indices, k):
			if key[2] == 'writers':
				# NR_open is writer when truncating/creating files
				if nr in [NR_read, NR_access,
						  NR_readlink, NR_dup2, NR_close, NR_chdir,
						  NR_munmap, NR_wait4]:
					continue
				# XXX
 				if nr == NR_execve and offset > 100000:
					continue
#				if nr not in [NR_write]:
#					print 'writer syscall?', nr, syscalls[nr].name, 'for key', keytype, key, '@', offset, fidx
			if key_sid and (key_sid & 0xffffffff) != sid: continue
			p = (fidx, offset)
			offsets.add(p)

		if keytype == 'ino' and key[2] != 'writers':
			xorkey = xorkey ^ 0x80000000
			k = struct.pack("I", xorkey)
			for (fidx, offset, nr, sid) in self._dbget(indices, k):
				p = (fidx, offset)
				offsets.add(p)

		if pathids is not None and len(pathids) != 0 and key[2] != 'writers':
		...
		if keytype != 'pid' or key_sid:
			for pid in set([r.pid for r in records]):
