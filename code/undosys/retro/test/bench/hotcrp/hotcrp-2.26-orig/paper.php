...
require_once("Code/resinhelpers.inc");

stats_begin ('getenv');

$nauth = getenv("NAUTH");
$npc = getenv("NPC");
$nchair = getenv("NCHAIR");
$sessname = getenv("SESSNAME");

if ($npc && $nchair) { 
  // Randomly select an PC member 
  $sid = $nchair + rand(1, $npc);
  session_id("$sessname-contact$sid");
}
if ($nauth) {
  // Randomly set paper id
  $_REQUEST['p'] = rand (1, $nauth);
}
stats_end ('getenv');

stats_begin ('read_header');
require_once("Code/header.inc");
stats_end ('read_header');
stats_begin ('read_papertable');
require_once("Code/papertable.inc");
stats_end ('read_papertable');
...
function confHeader() {
    ...
    //stats_begin("abar");
    $abar = actionBar($prow, $newPaper, $mode);
    //stats_end("abar");

    //stats_begin("confheader1");
    $Conf->header($title, "paper_" . ($mode == "pe" ? "edit" : "view"), $abar, false);
    //stats_end("confheader1");
...
stats_end ('paperactions');

  stats_begin ('withdraw');
...
stats_end ('update-etc');

stats_begin('paperactions2');
// paper actions
...
stats_end('paperactions2');

  stats_begin ('confheader');
// page header
confHeader();
  stats_end ('confheader');
